...
## 🆕 UPDATE 2026-07-15 — 3D "spin" ring + Spin/Grid toggle (READ THIS FIRST, it overrides older notes below)

The gallery was redesigned. Key changes — where these conflict with the 2026-05-24 notes further down, **this section wins**:

- **Two views, user-toggleable.** A **SPIN / GRID** toggle sits under the title. **Spin** = a 3D elliptical ring of all paintings (drag to spin, ‹ › arrows, gentle auto-rotate, front painting enlarged in the center with a "CLICK TO ENLARGE +" that opens the lightbox). **Grid** = the original CSS-columns masonry. Default is **Spin on desktop (≥900px), Grid on iPhone**; the visitor's pick is saved per-device in `localStorage` (`view` key). CSS drives it via `body[data-view="spin"|"grid"]`.
- **Gallery is now data-driven.** `_src/gallery.html` holds a single `PAINTINGS = [{slug,title}, …]` array that builds ring + grid + lightbox in JS. `publish.sh` appends new entries to that array **just before the `// __PAINTINGS_END__` marker** (it no longer splices static `<button class="tile">` blocks before `</main>`). Detection grep is now `slug: "<slug>"`.
- **New image variant: `<slug>-thumb.jpg`** (~440px, quality 72) drives the ring — 51 of them ≈ 2.7 MB total, so the ring doesn't load 51× 2200px JPGs. `build.sh` now emits four outputs per painting (avif, jpg, hires, **thumb**). The initial 51 thumbs were generated from the already-committed 2200px JPGs with `sips` (no originals needed).
- **`_src/gallery.html` IS NOW COMMITTED** (reversing the old "never commit _src" rule). Rationale: the user works across two Macs and wanted `git pull` to sync the gallery source; the images are already public in `images/` so committing the unencrypted gallery leaks essentially nothing new, and the live `index.html` stays Staticrypt-encrypted regardless. `.gitignore` now ignores only `_src/index.html` (the transient encrypt temp), not the whole `_src/` dir. **Do not re-add `_src/` to `.gitignore`** — it would silently desync the two Macs. `originals/` and `.password` remain gitignored.
- Live + verified 2026-07-15 (HTTP 200, encryption intact). Commit `a36f6d1`.
...
- ~~The unencrypted source (`_src/gallery.html`) must NEVER be committed.~~ **SUPERSEDED 2026-07-15 — `_src/gallery.html` is now committed on purpose** (see the "UPDATE 2026-07-15" section at the top for the full rationale). The images are already public in `images/`, so committing the gallery source leaks essentially nothing new, and `index.html` stays encrypted. Only `_src/index.html` (the transient encrypt temp) stays gitignored.
...
- **Never commit `originals/`** (still gitignored). `_src/gallery.html` **is** committed as of 2026-07-15 (see top); only `_src/index.html` (transient) is ignored. `publish.sh` is careful but verify with `git status` if anything looks off.
